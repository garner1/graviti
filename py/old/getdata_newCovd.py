...
def covd_rotInv(mat):
    ...
    for ind in range(len(I)):
        ...
        idx_1nn = np.intersect1d(idx_nnx,idx_nny) 
        delta_1nn = I[idx_1nn]-I[ind]
        ...
        idx_2nn = np.intersect1d(idx_nnx,idx_nny)
        delta_2nn = I[idx_2nn]-I[ind]
        ...
        idx_3nn = np.intersect1d(idx_nnx,idx_nny)
        delta_3nn = I[idx_3nn]-I[ind]
...
for region in regionprops(mask_label,intensity_image=dapi_fov):
    ...
    if np.count_nonzero(region.intensity_image) <= 1:        #at least 1 cell
        ...
        descriptors.append(covd_rotInv(region.intensity_image))
#save covd to file
if numb_of_nuclei > 0:
